...
public class FreeBoardService {
	...
	//
	public ArrayList<FreeBoard> selectSearch(String type, String keyword, int currentPage, int limit) {
		Connection con = getConnection();
		ArrayList<FreeBoard> list = new FreeBoardDao().selectTitleSearch(con, type, keyword, currentPage, limit);
	...
	public int getSearchListCount(String type, String keyword) {
		...
		System.out.println("Service단 type :" + type);
	...
	public int deleteCommentBoard(int commentNum) {
		Connection con = getConnection();
		int result = new FreeBoardDao().deleteCommentBoard(con, commentNum);
		if(result > 0 ) {
			commit(con);
	...
	public ArrayList<FreeBoard> selectTop3() {
		Connection con = getConnection();
		ArrayList<FreeBoard> list = new FreeBoardDao().selectTop3(con);
		close(con);
		return list;
	}
